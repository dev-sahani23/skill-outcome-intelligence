...
export function ProviderReport({ data }: { data: any }) {
  if (!data) return null;
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="indus-panel p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-foreground flex items-center gap-2">
            🏢 Provider Report — {data.subject.name}
          </h2>
          <p className="text-muted-foreground mt-1">
            {data.subject.district || "Unknown District"}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <ReportMetricCard 
          ...
          value={data.enrollment.completionRate !== null ? `${data.enrollment.completionRate}%` : "N/A"} 
          subtitle="overall"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="indus-panel p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center bg-chassis p-4 rounded-lg border border-border">
              <span className="text-sm font-medium text-muted-foreground uppercase">Total Placed / Not Placed</span>
              <span className="font-bold text-lg indus-number">
                {data.placement.placed} / {data.placement.notPlaced}
              </span>
            </div>
          </div>
